function [x,y] = snakedeform_ex(x,y,alpha,beta,gamma,kappa,fx,fy,ITER)
...
alpham1 = [alpha(2:N) alpha(1)]; % alfa(i+1)
alphap1 = [alpha(N) alpha(1:N-1)]; %alfa(i-1)
betam1 = [beta(2:N) beta(1)]; %beta(i+1)
betap1 = [beta(N) beta(1:N-1)]; %beta(i-1)
...
for count = 1:ITER,
   ...
   vfy = interp2(fy,x,y,'*linear');  %pongo la cubica que no me da problemas al elegir el contorno inicial
